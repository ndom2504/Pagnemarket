...
type Props = {
  ...
  max?: number;
  /** "avatar" renders a single round picture with a camera badge (replaces on pick). */
  variant?: "grid" | "avatar";
  fallbackUri?: string;
};
...
export function PhotoPicker({ images, onChange, max = 5, variant = "grid", fallbackUri }: Props) {
  ...
  const launch = async (source: Source) => {
    ...
    try {
      ...
      onChange(isAvatar ? urls.slice(0, 1) : [...images, ...urls]);
    } catch (e: any) {
      ...
      {isAvatar ? (
        <View style={{ alignItems: "center", gap: 10 }}>
          <Pressable testID="avatar-pick" style={styles.avatarWrap} onPress={() => setSheet(true)} disabled={uploading}>
            <Image
              source={{ uri: images[0] || fallbackUri }}
          ...
          {images[0] && (
            <Pressable testID="avatar-remove" onPress={() => onChange([])} style={{ padding: 4 }}>
